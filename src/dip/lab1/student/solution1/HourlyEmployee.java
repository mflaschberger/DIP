 ...
 * method. The other were not chosen for various reasons. Usually you will have
 * many methods that are common. Also, notice that we chose the identifier
 * "getAnnualWages" which works for both SalariedEmployee and HourlyEmployee.
 * That's significant because we had to make a conscious decision to rename the
 * original SalariedEmployee method, which was getAnnualSalary() to something
 * that works for all sub classes. You'll need to consider the names you give to
 * your common method vary carefully.
 *
 * @author Matthew Flaschberger
 */
public class HourlyEmployee implements dip.lab1.instructor.solution1.Employee {
